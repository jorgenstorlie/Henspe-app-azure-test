...
namespace Henspe.Core.Storage 
{
	...
	/// </summary>
	public class Connection : SQLiteBase.SQLiteConnection 
	{
		private bool _open;
		...
		/// </param>
		public Connection(string databasePath) : base (databasePath)
		{
		...
		/// </returns>
		public override int CreateTable<T>()
		{
			var ty = typeof(T);
		...
		/// </returns>
		public override SQLiteBase.SQLiteCommand CreateCommand(string cmdText, params object[] ps)
		{
			if (!_open) 
			{
		...
		public override int Execute(string query, params object[] args)
		{
			...
			int r = (cmd as SQLiteCommand).ExecuteNonQuery();
		...
		{
			var cmd = CreateCommand(query, args);
			return (cmd as SQLiteCommand).ExecuteQuery<T>();
		}
		...
		{
			var cmd = CreateCommand(query, args);
			return (cmd as SQLiteCommand).ExecuteDeferredQuery<T>();
		}
		...
		/// An enumerable with one result for each row returned by the query.
		/// </returns>
		public override List<object> Query(SQLiteBase.TableMapping map, string query, params object[] args)
		{
			var cmd = CreateCommand(query, args);
		...
		/// connection must remain open for the lifetime of the enumerator.
		/// </returns>
		public override IEnumerable<object> DeferredQuery(SQLiteBase.TableMapping map, string query, params object[] args)
		{
			var cmd = CreateCommand(query, args);
			return (cmd as SQLiteCommand).ExecuteDeferredQuery<object>(map);
		}
		...
		/// </returns>
		public override SQLiteBase.TableQuery<T> Table<T>() 
		{
		...
		/// </returns>
		public override T Get<T>(object pk)
		{
		...
		/// </returns>
		public override int InsertAll(System.Collections.IEnumerable objects)
		{
			BeginTransaction();
		...
		public override int Insert(object obj, Type objType)
		{
		...
		/// The number of rows added to the table.
		/// </returns>
		public override int Insert(object obj, string extra, Type objType)
		{
		...
		public override int Update(object obj, Type objType)
		{
			if (obj == null || objType == null) 
			{
				return 0;
    ...
    public class TableMapping : SQLiteBase.TableMapping
    {
        ...
        public PreparedSqlLiteInsertCommand GetInsertCommand(SQLiteBase.SQLiteConnection conn, string extra)
        {
        ...
        private PreparedSqlLiteInsertCommand CreateInsertCommand(SQLiteBase.SQLiteConnection conn, string extra)
        {
    ...
    public class SQLiteCommand : SQLiteBase.SQLiteCommand
    {
        Connection _conn;
        ...
        internal SQLiteCommand(Connection conn)
        {
            _conn = conn;
        ...
        public List<T> ExecuteQuery<T>(SQLiteBase.TableMapping map)
        {
            return ExecuteDeferredQuery<T>(map).ToList();
        }

        public IEnumerable<T> ExecuteDeferredQuery<T>(SQLiteBase.TableMapping map1)
        {
            TableMapping map = (TableMapping)map1;
            if (_conn.Trace)
            {
#if DEBUG
    ...
    public class PreparedSqlLiteInsertCommand : IDisposable
    {
        ...
        protected SQLiteBase.SQLiteConnection Connection { get; set; }
        ...
        internal PreparedSqlLiteInsertCommand(SQLiteBase.SQLiteConnection conn)
        {
            Connection = conn;
        }
        ...
        public int ExecuteNonQuery(object[] source)
        {
            ...
            if (r == SQLite3.Result.Done)
            {
                int rowsAffected = SQLite3.Changes((Connection as Connection).Handle);
                SQLite3.Reset(Statement);
                return rowsAffected;
            }
            else if (r == SQLite3.Result.Error)
            {
                string msg = SQLite3.GetErrmsg((Connection as Connection).Handle);
                SQLite3.Reset(Statement);
                throw SQLiteException.New(r, msg);
            }
            else
        ...
        {
            var stmt = SQLite3.Prepare2((Connection as Connection).Handle, CommandText);
            return stmt;
        }
    ...
    public class TableQuery<T> : SQLiteBase.TableQuery<T>, System.Collections.IEnumerable where T : new()
    {
        public SQLiteBase.SQLiteConnection Connection { get; private set; }
        ...
        TableQuery(SQLiteBase.SQLiteConnection conn, TableMapping table)
        {
            Connection = conn;
            Table = table;
        }

        public TableQuery(SQLiteBase.SQLiteConnection conn)
        {
            Connection = conn;
            Table = (Connection as Connection).GetMapping(typeof(T));
        }
        ...
        private SQLiteCommand GenerateCommand(string selectionList)
        {
            ...
            return (SQLiteCommand)(Connection as Connection).CreateCommand(cmdText, args.ToArray());
        }
